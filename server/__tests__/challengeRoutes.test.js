...
describe("Validating post challenge/:id/habits routes", () => {
    ...
    it("should successfully add correct number of habits", async () => {
        // Create Challenge 
        mockPrisma.challenges.findFirst.mockResolvedValue({id:1, challenge_name: "Test challenge", user_id:userId, total_habits:3, current_day:1, status:"active"});
        mockPrisma.challenge_habits.findMany.mockResolvedValue(null);
        mockPrisma.challenge_habits.createMany.mockResolvedValue([
            {id:10, challenge_id:1, habit_name:"Test Habit 1", habit_order: 1 },
            {id:11, challenge_id:1, habit_name:"Test Habit 2", habit_order: 2 },
            {id:12, challenge_id:1, habit_name:"Test Habit 3", habit_order: 3}
        ])
        const habits = await request(mockapp).post("/challenges/1/habits")
            .set("Authorization", `Bearer ${token}`)
            .send([
        ...
        console.log("habits", habits || null);
        ...
        expect(data[0].challenge_id).toBe(1);
    })
    it("should reject if challenge belongs to another user (403)", async () => {
        mockPrisma.challenges.findFirst.mockResolvedValue({id:1, challenge_name: "Test challenge", user_id:2, total_habits:3, current_day:1, status:"active"});
        mockPrisma.challenge_habits.findMany.mockResolvedValue(null);
        const response = await request(mockapp).post("/challenges/1/habits")
            .set("Authorization", `Bearer ${token}`)
            .send([
                {challenge_id:1, habit_name:"Test Habit 1", habit_order: 1},
                {challenge_id:1, habit_name:"Test Habit 2", habit_order: 2},
    ...
    it("should reject if challenge doesn't exist (404)", async () => {
        mockPrisma.challenges.findFirst(null);
        const response = await request(mockapp).post("/challenges/1/habits")
        .set("Authorization", `Bearer ${token}`)    
        .send([
                {challenge_id:1, habit_name:"Test Habit 1", habit_order: 1},
    ...
    it("should reject if wrong number of habits provided", async () => {
        mockPrisma.challenges.findFirst.mockResolvedValue({id:1, challenge_name: "Test challenge", user_id:userId, total_habits:4, current_day:1, status:"active"});
        ...
        const response = await request(mockapp).post("/challenges/1/habits")
            ...
            .send([
                ...
                {challenge_id:1, habit_name:"Test Habit 3", habit_order: 3}])
        expect(response.status).toBe(400);
        expect(response.body.error.message).toBe("Habits provided do not match the total habits from the challenge")
    })
    it("should reject if habits already added",  async () => {
        mockPrisma.challenges.findFirst.mockResolvedValue({id:1, challenge_name: "Test challenge", user_id:userId, total_habits:3, current_day:1, status:"active"});
        mockPrisma.challenge_habits.findMany.mockResolvedValue([{id: 11, challenge_id:1, habit_name:"Test Habit 1", habit_order: 1}]);
        ...
        expect(response.body.error.message).toBe("Habits already added to the challenge")
    })
    it("should reject if challenge not active", async () =>{
        mockPrisma.challenges.findFirst.mockResolvedValue({id:1, challenge_name: "Test challenge", user_id:userId, total_habits:3, current_day:1, status:"completed"});
        mockPrisma.challenge_habits.findMany.mockResolvedValue(null);
        const response = await request(mockapp).post("/challenges/1/habits")
            .set("Authorization", `Bearer ${token}`)
            .send([
                ...
                {challenge_id:1, habit_name:"Test Habit 3", habit_order: 3}])
        expect(response.status).toBe(400);
        expect(response.body.error.message).toBe("Can't add habits to inactive challenges")
    })
    it("should reject if empty habit Names", async () => {
        const response = await request(mockapp).post("/challenges/1/habits")
            ...
            .send([
                ...
                {challenge_id:1, habit_name:"Test Habit 3", habit_order: 3}])
        expect(response.status).toBe(400);
        expect(response.body.error.message).toBe("Missing or empty habit name found")
    })

    it.only("should reject empty habits array", async () => {
        const response = await request(mockapp).post("/challenges/1/habits")
            .set("Authorization", `Bearer ${token}`)
            .send([])
        expect(response.status).toBe(400);
        expect(response.body.error.message).toBe("Need to provide habits for the challenge");
    })
  
    
    // **Error Cases**:
    // 6. Rejects unauthenticated request (401)
    // 7. Rejects if challenge belongs to other user (403)
    // 8. Rejects if challenge doesn't exist (404)
    // 9. Rejects if wrong number of habits (400)
    // 10. Rejects if habits already added (409)
    // 11. Rejects if challenge not active (400)
    // 12. Rejects empty habit names
    // 13. Rejects if habits array empty

    // **Data Integrity**:
    // 14. Habits appear in database after creation
    // 15. habit_order is sequential
    // 16. All habits linked to correct challenge_id
